...
// Ativar botão de envio só quando o checkbox do termo está marcado
const termoCheck = document.getElementById('termoCheck');
const submitBtn = document.getElementById('submitBtn');
termoCheck.addEventListener('change', () => {
  submitBtn.disabled = !termoCheck.checked;
  submitBtn.style.opacity = termoCheck.checked ? '1' : '0.5';
  submitBtn.style.cursor = termoCheck.checked ? 'pointer' : 'not-allowed';
});
...
document.getElementById('bookingForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const CALLMEBOT_PHONE  = 'SEU_NUMERO';   // ex: 351912345678
    const CALLMEBOT_APIKEY = 'SUA_APIKEY';   // chave recebida do CallMeBot
    ...
    msgEl.textContent = 'Vamos responder ainda hoje com uma proposta para a vossa festa. Entretanto, já podes ir treinando o parabéns.';
