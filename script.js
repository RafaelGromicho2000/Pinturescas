...
const PRICE_DATA = {
  ...
  insuflavel: {
    ...
    note: 'Serviço de montagem e desmontagem'
  },
  jogos: {
...
const SERVICE_OPTIONS = {
  ...
  insuflavel: { label: 'Insuflável', sections: [
    { id: 'dur', label: 'Duração', multi: false, opts: ['3h','6h'] }
  ]},
  jogos:      { label: 'Jogos', sections: [
    { id: 'tipo', label: 'Jogos pretendidos', multi: true,  opts: ['Jogo das cadeiras','Jogo do galo dinâmico','Jogo da raposa','Estafetas'] },
      ...
      sec.opts.forEach(opt => {
        ...
        pill.addEventListener('click', () => {
          if (sec.multi) {
            ...
            serviceSelections[key][sec.id] = opt;
            grp.querySelectorAll('.opt-pill').forEach(p => p.classList.remove('selected'));
            pill.classList.add('selected');
          }
        });
  ...
  try {
    ...
    try {
      const msg = encodeURIComponent(
        `Novo pedido Pinturescas! Nome: ${name} | Tel: ${phone} | Data: ${form.date.value || '?'} | Servicos: ${selectedServices}`
      );
